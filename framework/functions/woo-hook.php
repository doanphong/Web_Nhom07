...
?>	
	<p class="availability stock <?php echo esc_attr($_product_stock['class']);?>"><span class="wd_availability"><?php _e('Tình Trạng:','wpdance'); ?></span><span><?php echo esc_attr($_product_stock['availability']);?></span></p>	
<?php	
...
function button_add_to_card(){
	...
	if($_left_sidebar || $_right_sidebar) {
		...
		<?php  } else {
			...
			echo apply_filters('single_add_to_cart_text', __( 'Thêm Vào Giỏ Hàng', 'wpdance' ), $product->product_type); 
			echo '</button>';
		}
	}	
}
...
function update_add_to_cart_text( $button_text ){
	return $button_text = __('Thêm Vào Giỏ Hàng','wpdance');
}
